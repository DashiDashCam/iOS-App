...
class PhotoManager: NSObject {
    ...
    public func getAssetThumbnail(asset: PHAsset) -> UIImage {
        ...
        manager.requestImage(for: asset, targetSize: CGSize(width: 360.0, height: 135.0), contentMode: .aspectFill, options: options) { result, _ in
            //
            if let thumb = result {
                thumbnail = thumb
            } else {
    ...
    public func fetchAVAssetForPHAsset(videoAsset: PHAsset, completion: @escaping (Bool, URL) -> Void) {
        ...
        PHImageManager.default().requestAVAsset(forVideo: videoAsset, options: options) { asset, _, _ in
            //
